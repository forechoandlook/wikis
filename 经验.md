# 【CANN，计算机视觉CV，TensorFlow】关于DPN和ResNext分割网络训练调优的一点心得


## 1. 背景 

昇腾910 AI处理器是华为在2019年发布的人工智能（AI）专用的神经网络处理器，其算力高达256T，最新款算力高达310T，是业界主流芯片算力的2倍，可对接MindSpore/TensorFlow/Pytorch等训练框架。当前业界大多数训练脚本基于TensorFlow的Python API开发，默认运行在CPU/GPU/TPU上。为了使其能够利用昇腾910 AI处理器的澎湃算力执行训练，提升训练性能，我们需要对训练网络脚本进行相关的迁移适配工作。  
...
原始论文中设计的网络结构需要占用非常大的内存空间，主要是第8层卷积（`5*5*512`）、第9层卷积（`25*25*4096`），第12层局部卷积（`50*50*21`）和第13层卷积（`9*9*105`），在双精度下，这几个卷积层占了总内存的90%以上。鉴于NPU运行内存的限制（针对不同网络模型，内存分配可以通过配置进行适当调整），以及模型并行的改造工作量，为了在单个NPU上能够正常训练，对这几个卷积层进行了适当的修改，达到参数降维的目的。  
...
### 数据并行的分布式训练 
我们复现的是`ResNeXt+DeeplabV3`做分割的模型。原论文使用8机64卡分布式训练，由于硬件条件限制，我们最终决定采用单机8卡分布式训练(感谢官方直接提供了一台8卡裸机给我们）。npu上进行分布式训练与gpu上存在较大差异。我们依据官方教程，并根据自己的实践做了小结，被modelzoo社区吸收为wiki，链接如下。   
